...
intensity = cp.zeros((Nz, Nx))   # 初始化光强数组

# 生成频域坐标网格 (直接使用 fftfreq，零频自动在原点)
fx = cp.fft.fftfreq(Nx, d=dx0)
fy = cp.fft.fftfreq(Ny, d=dx0)
...
for iz in range(Nz):
    ...
    if cp.isclose(z_current, 0):
        ...
        # 用这个会得到一个异常锐利、甚至小于物理衍射极限的错误光斑，这正是想要的证明！
        phase_fresnel = 1j * k * z_current - 1j * np.pi * lamda * z_current * (FX**2 + FY**2)
        # $$H(f_x, f_y) = \exp(j k z) \cdot \exp[-j \pi \lambda z (f_x^2 + f_y^2)]$$
        H = cp.exp(phase_fresnel)
        # 频域相乘后直接 IFFT 回到空域
        U = cp.fft.ifft2(U0_fft * H)

    # 计算光强，并提取y=0截面
    intensity_full = cp.abs(U)**2
